...
export const WarningAlert = ({ message, className }: IWarningAlertProps) => {
  return (
    <div
      id="alert-border-5"
      className={`flex p-4 mb-4  bg-yellow-100 border-t-4 border-yellow-500 dark:bg-yellow-200 ${className}`}
      role="alert"
    >
      <svg
      ...
      <div className="ml-3 text-sm font-medium text-yellow-700">{message}</div>
      <button
        type="button"
